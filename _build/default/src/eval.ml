...
let rec eval_expr env t =
  ...
  | Add (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                          let n2 = eval_expr env expr2 in 
                          (match n1, n2 with
                          | (Int_Val x), (Int_Val y) -> Int_Val (x+y)
                          | _ -> raise (TypeError "Wrong types!"))
  | Sub (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                          let n2 = eval_expr env expr2 in 
                          (match n1, n2 with
                          | (Int_Val x), (Int_Val y) -> Int_Val (x-y)
                          | _ -> raise (TypeError "Wrong types!"))
  | Mult (expr1, expr2) ->  let n1 = eval_expr env expr1 in 
                            let n2 = eval_expr env expr2 in 
                            (match n1, n2 with
                            | (Int_Val x), (Int_Val y) -> Int_Val (x*y)
                            | _ -> raise (TypeError "Wrong types!"))
  | Div (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                          let n2 = eval_expr env expr2 in 
                          (match n1, n2 with
                          | (Int_Val x), (Int_Val 0) -> raise DivByZeroError
                          | (Int_Val x), (Int_Val y) -> Int_Val (x/y)
                          | _ -> raise (TypeError "Wrong types!"))
  | Pow (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                          let n2 = eval_expr env expr2 in 
                          (match n1, n2 with
                          | (Int_Val x), (Int_Val 0) -> Int_Val (1)
                          | (Int_Val x), (Int_Val y) -> Int_Val (int_of_float((float_of_int x)**(float_of_int y)))
                          | _ -> raise (TypeError "Wrong types!"))
  | Greater (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                              let n2 = eval_expr env expr2 in 
                              (match n1, n2 with
                              | (Int_Val x), (Int_Val y) -> if x > y then Bool_Val true else Bool_Val false
                              | _ -> raise (TypeError "Wrong types!"))
  | Less (expr1, expr2) ->  let n1 = eval_expr env expr1 in 
                            let n2 = eval_expr env expr2 in 
                            (match n1, n2 with
                            | (Int_Val x), (Int_Val y) -> if x < y then Bool_Val true else Bool_Val false
                            | _ -> raise (TypeError "Wrong types!"))
  | GreaterEqual (expr1, expr2) ->  let n1 = eval_expr env expr1 in 
                                    let n2 = eval_expr env expr2 in 
                                    (match n1, n2 with
                                    | (Int_Val x), (Int_Val y) -> if (x >= y) then Bool_Val true else Bool_Val false
                                    | _ -> raise (TypeError "Wrong types!"))
  | LessEqual (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                                let n2 = eval_expr env expr2 in 
                                (match n1, n2 with
                                | (Int_Val x), (Int_Val y) -> if (x <= y) then Bool_Val true else Bool_Val false
                                | _ -> raise (TypeError "Wrong types!"))
  | Equal (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                            let n2 = eval_expr env expr2 in 
                            (match n1, n2 with
                            | (Int_Val x), (Int_Val y) -> if x = y then Bool_Val true else Bool_Val false
                            | (Bool_Val x), (Bool_Val y) -> if x = y then Bool_Val true else Bool_Val false
                            | _ -> raise (TypeError "Wrong types!"))
  | NotEqual (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                            let n2 = eval_expr env expr2 in 
                            (match n1, n2 with
                            | (Int_Val x), (Int_Val y) -> if x <> y then Bool_Val true else Bool_Val false
                            | (Bool_Val x), (Bool_Val y) -> if x <> y then Bool_Val true else Bool_Val false
                            | _ -> raise (TypeError "Wrong types!"))
  | Or (expr1, expr2) ->  let n1 = eval_expr env expr1 in 
                          let n2 = eval_expr env expr2 in 
                          (match n1, n2 with
                          ...
                          | _ -> raise (TypeError "Wrong types!"))
  | And (expr1, expr2) -> let n1 = eval_expr env expr1 in 
                          let n2 = eval_expr env expr2 in 
                          (match n1, n2 with
                          | (Bool_Val true), (Bool_Val true) -> Bool_Val true
                          ...
                          | (Bool_Val false), (Bool_Val false) -> Bool_Val false
                          | _ -> raise (TypeError "Wrong types!"))
  | Not expr -> let n1 = eval_expr env expr in 
                (match n1 with
